...
def export_pdf(report, run_ts):
    ...
    for item in report:
        ...
        story.append(Paragraph(
            f"<b>NVD:</b> {str(item['nvd_correlation'])[:1000]}",
            styles["Normal"]
        ))
...
def main():
    ...
    print(f"JSON (appended): {json_path}")
    print(f"PDF (new): {pdf_path}")
